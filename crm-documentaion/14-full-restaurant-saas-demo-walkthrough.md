...
| 5    | Follow-up console error showed `/api/media/assets/{uuid}` returning 404 after `Create graphic`. Root cause: generated media asset upserts could keep an existing DB row id while overwriting `publicUrl` with a new random UUID, producing URLs with no matching row. Fix implemented: generated asset registration now canonicalizes `publicUrl` to the actual returned asset id, and the media route can recover existing broken rows by looking up the requested public URL.           | Blocking  | 2026-07-06 media asset 404 console             | Fixed; redeploy and retest |
| 5    | A later production retest still showed `/api/media/assets/{uuid}` 404. Hardening added: the media route redirects missing legacy social creative asset URLs to the stable social creative renderer, and the social post editor/list hide media URLs after an image load error so an old broken generated image cannot keep blocking the designed graphic preview.                                                                                                                         | Blocking  | 2026-07-06 repeated media asset 404 console    | Fixed; redeploy and retest |
| 5    | Production then reached `/api/social-creatives/{jobId}/image` but the route returned HTML 500, causing the editor to display a render-failed error. Fix implemented: the social creative image route now catches `ImageResponse` renderer failures and returns a simple branded SVG fallback image instead of HTML 500; the worker only persists rendered assets when the route returns real PNG, avoiding bad stored assets from fallback responses.                                     | Blocking  | 2026-07-06 social creative image 500 console   | Fixed; redeploy and retest |
