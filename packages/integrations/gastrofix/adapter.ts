import { eq, and } from "drizzle-orm";
import type { Database } from "@marketing/db";
import { integrationConnections } from "@marketing/db";
import type { TenantContext } from "@marketing/tenancy";
import { logger } from "@marketing/shared";
import { encryptTokens, decryptTokens } from "../src/crypto";
import type {
  ConnectCredentials,
  IIntegrationAdapter,
  IntegrationConnection,
  SyncResult,
  WebhookEvent,
} from "../src/interface";
import { createGastrofixClient } from "./client";
import { verifyGastrofixSignature } from "./webhook";
import { GASTROFIX_SCOPES } from "./config";

export class GastrofixAdapter implements IIntegrationAdapter {
  readonly provider = "gastrofix" as const;
  readonly authType = "api_key" as const;

  constructor(
    private readonly db: Database,
    private readonly encKey: string,
  ) {}

  async connect(ctx: TenantContext, creds: ConnectCredentials): Promise<IntegrationConnection> {
    if (!creds.apiKey) throw new Error("Gastrofix connect requires an API key");

    const tokens = encryptTokens({ api_key: creds.apiKey }, this.encKey);
    const externalAccountId = creds.externalAccountId ?? "default";

    const [row] = await this.db
      .insert(integrationConnections)
      .values({
        tenantId: ctx.tenantId,
        provider: "gastrofix",
        externalAccountId,
        oauthTokens: tokens,
        scopes: GASTROFIX_SCOPES,
        status: "connected",
        meta: {},
      })
      .onConflictDoUpdate({
        target: [
          integrationConnections.tenantId,
          integrationConnections.provider,
          integrationConnections.externalAccountId,
        ],
        set: {
          oauthTokens: tokens,
          status: "connected",
          updatedAt: new Date(),
        },
      })
      .returning();

    return this.mapRow(row!);
  }

  async disconnect(ctx: TenantContext, connectionId: string): Promise<void> {
    await this.db
      .update(integrationConnections)
      .set({ status: "disconnected", updatedAt: new Date() })
      .where(
        and(
          eq(integrationConnections.id, connectionId),
          eq(integrationConnections.tenantId, ctx.tenantId),
        ),
      );
  }

  async sync(ctx: TenantContext, connection: IntegrationConnection): Promise<SyncResult> {
    const tokens = decryptTokens(connection.oauthTokens, this.encKey);
    const apiKey = tokens.api_key as string;
    if (!apiKey) throw new Error("Gastrofix connection missing api_key");

    const client = createGastrofixClient(apiKey);
    const today = new Date().toISOString().split("T")[0]!;

    try {
      const [reservations, menuItems] = await Promise.all([
        client.listReservations(today),
        client.listMenuItems(),
      ]);

      await this.db
        .update(integrationConnections)
        .set({
          lastSyncAt: new Date(),
          meta: { reservationCount: reservations.length, menuItemCount: menuItems.length },
          updatedAt: new Date(),
        })
        .where(
          and(
            eq(integrationConnections.id, connection.id),
            eq(integrationConnections.tenantId, ctx.tenantId),
          ),
        );

      const count = reservations.length + menuItems.length;
      logger.info(
        { tenantId: ctx.tenantId, provider: "gastrofix", connectionId: connection.id, count },
        "[gastrofix] sync completed",
      );
      return { outcome: "ok", recordsProcessed: count };
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
      logger.error(
        { tenantId: ctx.tenantId, provider: "gastrofix", connectionId: connection.id, err: errorMessage },
        "[gastrofix] sync failed",
      );
      await this.db
        .update(integrationConnections)
        .set({ status: "error", updatedAt: new Date() })
        .where(
          and(
            eq(integrationConnections.id, connection.id),
            eq(integrationConnections.tenantId, ctx.tenantId),
          ),
        );
      return { outcome: "error", recordsProcessed: 0, errorMessage };
    }
  }

  verifyWebhook(rawBody: string, signature: string, secret: string): boolean {
    return verifyGastrofixSignature(rawBody, signature, secret);
  }

  async processWebhookEvent(ctx: TenantContext, event: WebhookEvent): Promise<void> {
    const { processGastrofixEvent } = await import("./handlers");
    await processGastrofixEvent(ctx, event, this.db);
  }

  private mapRow(row: typeof integrationConnections.$inferSelect): IntegrationConnection {
    return {
      id: row.id,
      tenantId: row.tenantId,
      provider: row.provider,
      externalAccountId: row.externalAccountId,
      oauthTokens: row.oauthTokens,
      scopes: row.scopes ?? [],
      status: row.status as IntegrationConnection["status"],
      meta: (row.meta ?? {}) as Record<string, unknown>,
      connectedAt: row.connectedAt,
      lastSyncAt: row.lastSyncAt ?? null,
      updatedAt: row.updatedAt,
    };
  }
}
