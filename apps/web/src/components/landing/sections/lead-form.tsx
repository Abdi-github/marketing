  ...
  return (
    <section style={{ padding:"6rem 0", background:"#fff" }}>
      <div style={{ maxWidth:600, margin:"0 auto", padding:"0 1.5rem" }}>
        <p style={{ fontSize:"0.7rem", fontWeight:700, letterSpacing:"0.14em", textTransform:"uppercase", color:brandPrimary, marginBottom:"0.75rem", textAlign:"center" }}>Get in touch</p>
        <h2 style={{ fontFamily:"var(--font-heading,system-ui)", fontSize:"clamp(1.75rem,4vw,2.5rem)", fontWeight:800, color:"#111827", lineHeight:1.15, letterSpacing:"-0.02em", margin:"0 0 1rem", textAlign:"center" }}>{renderRich(section.heading)}</h2>
        {section.body && <p style={{ fontSize:"1rem", color:"#6b7280", lineHeight:1.8, textAlign:"center", maxWidth:500, margin:"0 auto 2.5rem" }}>{renderRich(section.body)}</p>}
        <div style={{ background:"#f9fafb", borderRadius:24, padding:"2.5rem", border:"1px solid #f3f4f6", boxShadow:"0 4px 24px rgba(0,0,0,0.04)", marginTop: section.body ? 0 : "3rem" }}>
          {formContent}
        </div>
      </div>
    </section>
...
export function LeadFormSplitSideImage({ section, brandPrimary, formContent }: Props) {
  return (
    <>
      <style>{`
        .lp-lfs { background:#f9fafb; padding:6rem 0; }
        .lp-lfs__inner { max-width:1100px; margin:0 auto; padding:0 1.5rem; display:flex; gap:4rem; align-items:center; }
        .lp-lfs__image { flex:1 1 48%; border-radius:24px; overflow:hidden; position:relative; min-height:480px; background:linear-gradient(135deg,#0f0c29,var(--brand-primary,#4f46e5)); }
        ...
        .lp-lfs__h2 { font-family:var(--font-heading,system-ui); font-size:clamp(1.75rem,4vw,2.5rem); font-weight:800; color:#111827; line-height:1.15; letter-spacing:-0.02em; margin:0 0 1rem; }
        .lp-lfs__body { font-size:0.95rem; color:#6b7280; line-height:1.8; margin:0 0 2rem; }
        .lp-lfs__card { background:#fff; border-radius:24px; padding:2.5rem; box-shadow:0 4px 24px rgba(0,0,0,0.06); border:1px solid #f0f0f0; }
      `}</style>
      <section className="lp-lfs">
        <div className="lp-lfs__inner">
          <div className="lp-lfs__image">
            <div style={{ position:"absolute", inset:0, background:`linear-gradient(135deg,${brandPrimary}dd,#0f0c29)`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"2.5rem", textAlign:"center" }}>
              <p style={{ fontFamily:"var(--font-heading,system-ui)", fontSize:"clamp(1.5rem,3vw,2.25rem)", fontWeight:800, color:"#fff", lineHeight:1.25, margin:"0 0 1rem" }}>{renderRich(section.heading)}</p>
              {section.body && <p style={{ fontSize:"0.95rem", color:"rgba(255,255,255,0.72)", lineHeight:1.75 }}>{renderRich(section.body)}</p>}
              <div style={{ marginTop:"2rem", display:"flex", gap:"1.5rem", justifyContent:"center" }}>
                {["✓ Free", "✓ Fast", "✓ No spam"].map((tag) => (
                  <span key={tag} style={{ fontSize:"0.8rem", color:"rgba(255,255,255,0.7)", fontWeight:600 }}>{tag}</span>
                ))}
              </div>
            </div>
          </div>
          <div className="lp-lfs__form">
            <div className="lp-lfs__card">
              ...
              <p className="lp-lfs__body">Fill in your details and we&apos;ll get back to you within 24 hours.</p>
              {formContent}
            </div>
          </div>
        </div>
