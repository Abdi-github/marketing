  ...
  return (
    <section style={{ padding:"6rem 1.5rem", background:"#f0fdf4" }}>
      <div style={{ maxWidth:560, margin:"0 auto", textAlign:"center" }}>
        <div style={{ width:72, height:72, borderRadius:"50%", background:WA_GREEN, color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 1.5rem", boxShadow:`0 8px 32px ${WA_GREEN}50` }}>
          <WAIcon />
        </div>
        <h2 style={{ fontFamily:"var(--font-heading,system-ui)", fontSize:"clamp(1.75rem,4vw,2.5rem)", fontWeight:800, color:"#111827", lineHeight:1.15, letterSpacing:"-0.02em", margin:"0 0 1rem" }}>
          {renderRich(section.heading)}
        </h2>
        {section.body && (
          <p style={{ fontSize:"1.05rem", color:"#374151", lineHeight:1.75, margin:"0 0 2.5rem" }}>{renderRich(section.body)}</p>
        )}
        <a
          href={waHref(section)}
          target="_blank"
          rel="noopener noreferrer"
          style={{ display:"inline-flex", alignItems:"center", gap:"0.75rem", padding:"1.1rem 2.5rem", borderRadius:9999, background:WA_GREEN, color:"#fff", fontWeight:700, fontSize:"1.1rem", textDecoration:"none", boxShadow:`0 8px 32px ${WA_GREEN}40` }}
        >
          <WAIcon />
          {buttonText}
        </a>
...
export function WhatsAppCtaBannerStrip({ section }: Props) {
  ...
  return (
    <section style={{ background:WA_DARK, padding:"1.75rem 0" }}>
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 1.5rem", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"2rem", flexWrap:"wrap" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"1rem" }}>
          <div style={{ color:"#fff", opacity:0.9, flexShrink:0 }}><WAIcon /></div>
          <div>
            <p style={{ fontWeight:700, fontSize:"1rem", color:"#fff", margin:0, lineHeight:1.3 }}>{renderRich(section.heading)}</p>
            {section.body && <p style={{ fontSize:"0.875rem", color:"rgba(255,255,255,0.7)", margin:"0.2rem 0 0", lineHeight:1.5 }}>{renderRich(section.body)}</p>}
          </div>
        </div>
        <a
          href={waHref(section)}
