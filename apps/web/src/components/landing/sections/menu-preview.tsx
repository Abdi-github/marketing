...
export function MenuPreviewListBorders({ section, brandPrimary }: Props) {
  ...
  return (
    <section style={{ padding: "6rem 0", background: "var(--lp-surface,#f9fafb)" }}>
        ...
        {items.length > 0 && (
          <div
            style={{
              ...
              border: "1px solid var(--lp-border,#f0f0f0)",
            }}
          >
            {items.map((item, i) => (
              <div
                ...
                style={{
                  ...
                  borderBottom:
                    i < items.length - 1 ? "1px solid var(--lp-subtle,#f3f4f6)" : "none",
                }}
              >
                <div style={{ flex: 1 }}>
                  <p
...
export function MenuPreviewCardsGrid({ section, brandPrimary }: Props) {
  ...
  return (
    <>
      <style>{`
        .lp-mcg { background:var(--lp-card,#fff); padding:6rem 0; }
        ...
        .lp-mcg__card { border-radius:18px; overflow:hidden; border:1px solid var(--lp-border,#f0f0f0); box-shadow:0 2px 12px rgba(0,0,0,0.04); background:var(--lp-card,#fff); }
        .lp-mcg__img { aspect-ratio:16/9; position:relative; background:var(--lp-subtle,#f3f4f6); overflow:hidden; }
        .lp-mcg__body { padding:1.25rem; }
        .lp-mcg__name { font-weight:700; font-size:1rem; color:var(--lp-text,#111827); margin:0 0 0.35rem; }
        .lp-mcg__desc { font-size:0.85rem; color:var(--lp-muted,#6b7280); line-height:1.6; margin:0 0 1rem; }
...
export function MenuPreviewSplitImage({ section, brandPrimary }: Props) {
  ...
  return (
    <>
      <style>{`
        .lp-msi { background:var(--lp-surface,#f9fafb); padding:6rem 0; }
        .lp-msi__inner { max-width:1100px; margin:0 auto; padding:0 1.5rem; display:flex; gap:4rem; align-items:center; }
        .lp-msi__image { flex:1 1 42%; border-radius:24px; overflow:hidden; position:relative; aspect-ratio:3/4; background:var(--lp-border,#e5e7eb); }
        ...
        .lp-msi__item { display:flex; align-items:flex-start; justify-content:space-between; gap:1rem; padding:1.1rem 0; border-bottom:1px solid var(--lp-border,#e5e7eb); }
        .lp-msi__item:last-child { border-bottom:none; }
        .lp-msi__item-name { font-weight:700; font-size:1rem; color:var(--lp-text,#111827); margin:0 0 0.25rem; }
        .lp-msi__item-desc { font-size:0.85rem; color:var(--lp-muted,#9ca3af); margin:0; line-height:1.5; }
            ...
            {heroImg ? (
              ...
              <div
                style={{
                  position: "absolute",
                  inset: 0,
                  background: `linear-gradient(135deg,${brandPrimary}18,${brandPrimary}06)`,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                }}
