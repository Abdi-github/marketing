...
type Props = { section: AboutSection; brandPrimary: string };
...
export function AboutTextImageSplit({ section, brandPrimary }: Props) {
  ...
  return (
    <>
      <style>{`
        .lp-ati { background:#fff; padding:6rem 0; }
        ...
        .lp-ati__image { flex:1 1 46%; border-radius:24px; overflow:hidden; position:relative; aspect-ratio:4/5; background:#f3f4f6; }
        ...
        .lp-ati__h2 { font-family:var(--font-heading,system-ui); font-size:clamp(1.75rem,4vw,2.75rem); font-weight:800; color:#111827; line-height:1.15; letter-spacing:-0.02em; margin:0 0 1.25rem; }
        .lp-ati__body { font-size:1rem; color:#6b7280; line-height:1.8; margin:0 0 2rem; }
        .lp-ati__checks { display:flex; flex-direction:column; gap:0.75rem; }
        .lp-ati__check { display:flex; align-items:flex-start; gap:0.75rem; font-size:0.95rem; color:#374151; }
        ...
        <div className="lp-ati__inner">
          <div className="lp-ati__text">
            ...
            <ul className="lp-ati__checks" style={{ listStyle:"none", margin:0, padding:0 }}>
              {(extras?.values ?? ["Quality craftsmanship", "Swiss reliability", "Personal service"]).slice(0, 4).map((v, i) => (
                <li key={i} className="lp-ati__check">
                  <span className="lp-ati__check-icon">
                    <svg width="10" height="10" viewBox="0 0 10 10" fill={brandPrimary} aria-hidden><path d="M8.5 2L4 7 1.5 4.5l-1 1L4 9 9.5 3z"/></svg>
                  </span>
                  {v}
                </li>
              ))}
...
export function AboutTeamGrid({ section, brandPrimary }: Props) {
  ...
  return (
    <>
      <style>{`
        .lp-atg { background:#f9fafb; padding:6rem 0; }
        ...
        .lp-atg__h2 { font-family:var(--font-heading,system-ui); font-size:clamp(1.75rem,4vw,2.75rem); font-weight:800; color:#111827; line-height:1.15; letter-spacing:-0.02em; margin:0 0 1rem; }
        .lp-atg__body { font-size:1rem; color:#6b7280; line-height:1.8; margin:0; }
        .lp-atg__grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); gap:1.5rem; }
        .lp-atg__card { background:#fff; border-radius:20px; padding:2rem 1.5rem; text-align:center; border:1px solid #f0f0f0; box-shadow:0 2px 12px rgba(0,0,0,0.04); }
        .lp-atg__avatar { width:80px; height:80px; border-radius:50%; object-fit:cover; margin:0 auto 1rem; display:block; }
        .lp-atg__avatar-initial { width:80px; height:80px; border-radius:50%; background:${brandPrimary}; color:#fff; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:1.75rem; margin:0 auto 1rem; }
        .lp-atg__name { font-weight:700; font-size:1rem; color:#111827; margin:0 0 0.25rem; }
        .lp-atg__role { font-size:0.8rem; color:#9ca3af; margin:0; }
      `}</style>
...
const VALUE_ICONS = ["⭐", "🛡️", "⚡", "🎯", "💡", "🤝"];
export function AboutValues3col({ section, brandPrimary }: Props) {
  const { heading, body, extras } = section;
  const values = extras?.values ?? ["Exceptional quality", "Swiss reliability", "Personal service"];
  return (
    <>
      <style>{`
        .lp-av3 { background:#fff; padding:6rem 0; }
        ...
        .lp-av3__h2 { font-family:var(--font-heading,system-ui); font-size:clamp(1.75rem,4vw,2.75rem); font-weight:800; color:#111827; line-height:1.15; letter-spacing:-0.02em; margin:0 0 1rem; }
        .lp-av3__body { font-size:1rem; color:#6b7280; line-height:1.8; margin:0; }
        .lp-av3__grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:2rem; }
        .lp-av3__item { padding:2rem; border-radius:20px; border:1px solid #f0f0f0; background:#fafafa; }
        .lp-av3__icon { font-size:2.25rem; margin-bottom:1.25rem; }
        .lp-av3__title { font-family:var(--font-heading,system-ui); font-size:1.1rem; font-weight:700; color:#111827; margin:0; }
      `}</style>
      <section className="lp-av3">
